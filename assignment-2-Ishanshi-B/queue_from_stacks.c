...
{
  // creating the stacks
  struct queue_from_stacks *qfs = (struct queue_from_stacks *)malloc(sizeof(struct queue_from_stacks));
  qfs->s1 = stack_create();
  qfs->s2 = stack_create();
  return qfs;
}
...
{
  // checking to see if the stack is empty for both the stacks that were created.
...
{
  // enque a new value with stack_push
...
void *queue_from_stacks_front(struct queue_from_stacks *qfs)
{
  ...
  {
    // push the the popped value from the first stack into the second stack
    stack_push(qfs->s2, stack_pop(qfs->s1));
  }

  // store the top value of the second stack
  int *frt = stack_top(qfs->s2);
  // repeat what was done in the stack in the first while just reversing the stacks
...
{
  // checking to see if the first stack is not empty
  while (!stack_isempty(qfs->s1))
  {
    // push the the popped value from the first stack into the second stack
    stack_push(qfs->s2, stack_pop(qfs->s1));
  }
  // store the popped value in an int variable
  int *frt = stack_pop(qfs->s2);
  // repeat the steps
  while (!stack_isempty(qfs->s2))
  {
    stack_push(qfs->s1, stack_pop(qfs->s2));
